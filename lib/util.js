...
// remove caracteres de controle C0 (exceto \t \n \r) — inclui \u0000 que quebra spawn
const CTRL_RE = /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g;

/** Normaliza mensagem de chat: remove controles, rejeita vazia e acima do limite. */
export function normalizeMessage(text) {
  const s = String(text ?? '').replace(CTRL_RE, '').trim();
  if (!s) {
...
/* ---------- menções como entidades (qualquer posição do texto) ---------- */
const MENTION_IN_RE = /(?<![\w@])@(linux|windows|todos|auto|pause|resume|status)/gi;
...
export function extractMentions(text) {
  ...
  const seen = new Set();
  let m;
  while ((m = MENTION_IN_RE.exec(s)) !== null) {
...
export function stripMentions(text) {
  return String(text ?? '').replace(MENTION_IN_RE, '').replace(/\s+/g, ' ').trim();
}